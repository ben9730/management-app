...
<task type="auto">
  ...
  <action>
    ...
    **Part B: Wire handleTaskFormSubmit with syncProgressAndStatus**
    ...
    1b. Ensure the `handleTaskFormSubmit` data parameter type includes `percent_complete?: number` (alongside existing fields like title, duration, etc.). This may require updating the TaskFormData type or inline type that handleTaskFormSubmit accepts.
