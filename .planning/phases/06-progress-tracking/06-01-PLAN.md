...
depends_on: []
files_modified:
  - supabase/migrations/008_add_progress_tracking.sql
...
must_haves:
  ...
  artifacts:
    ...
    - path: "flowplan/src/services/progress-sync.test.ts"
      ...
      min_lines: 80
    - path: "supabase/migrations/008_add_progress_tracking.sql"
...
<task type="auto">
  ...
  <action>
    1. Create migration `supabase/migrations/008_add_progress_tracking.sql`:
       ```sql
       ALTER TABLE tasks
  ...
  <verify>
    - Migration file exists at `supabase/migrations/008_add_progress_tracking.sql`
