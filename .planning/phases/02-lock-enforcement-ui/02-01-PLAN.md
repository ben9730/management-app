...
4. **Sidebar lock guards:** In the `DashboardContent` component body (NOT inside JSX), add a derived const near the other hook calls and derived state (before the `return` statement):
   ```typescript
   const isSelectedTaskLocked = selectedTask ? isLocked(selectedTask.phase_id || '') : false
   ```
   This must be placed in the component function body alongside other derived values -- it is a plain TypeScript const, not JSX. Then, in the Task Detail Sidebar JSX section (around line 1016-1030), use this pre-computed value:
   - On the delete button, add `disabled={deleteTaskMutation.isPending || isSelectedTaskLocked}`.
   - On the edit button, add `disabled={isSelectedTaskLocked}`.
   - Add `opacity-50 cursor-not-allowed` styling when disabled.
   Since page.tsx is already 1,238 lines, keep changes minimal.
  </action>
  <verify>
